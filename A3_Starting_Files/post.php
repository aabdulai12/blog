...
if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $stmt = $pdo->prepare("SELECT title, content, timestamp FROM posts WHERE id = ?");
    $stmt->execute([$id]);
...
<body>
    ...
    <div class="container">
        <?php if (isset($post)): ?>
            <div class="blog-post">
                ...
                <p><?php echo date("F d, Y, h:i a", strtotime($post['timestamp'])); ?></p>
    ...
    <footer class ="footer">
    <small class ="d-block text-end container">CopyMe &copy; <?=date('Y')?> * Built by Abubakar Abdulai!</small>
</footer>
</body>
</html>
